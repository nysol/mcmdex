...
int KGMACE::setArgs (int argc, char *argv[]){
  ...
  if ( strchr (argv[c], '_') ){ _onMsg  = false; }
  else                        { _sgFlag |= SHOW_MESSAGE;}
  if ( strchr (argv[c], '%') ){ _onProgress = true;}
  if ( strchr (argv[c], '+') ){ _outApend= true;}
...
void KGMACE::clq_iter ( QUEUE_INT v, QUEUE *occ){
  ...
  if (_itemset.get_t() >= _ub ) goto END;  // upper bound of clique
...
void KGMACE::add_vertex (QUEUE_INT K, QUEUE_INT v){
  ...
  if ( _problem & PROBLEM_CLOSED ){
    if ( _OQ[K].get_t() > VBMINT_MAX ) _problem -= PROBLEM_CLOSED; //ここ後で戻さなくてOK？
...
LONG KGMACE::parent_check ( QUEUE_INT K, QUEUE_INT w){
  ...
  QUEUE_INT *y  = _SG.edgeEnd(w) - 1 ;   // _SG.edge_vv(w) + _SG.edge_vt(w)-1;
  ...
  for(i=0;i<_OQ[w].get_t();i++){
		...
		e = _OQ[w].get_v(i);
		// pointers to the positions of the current 
		// processing items in each transaction
  ...
  for (x=_SG.edgeEnd(v)-1 ; *x>w ; x--){
    ...
    if ( *x <= (e=_OQ[K].get_v(j)) ){ 
      ...
      if ( zz-_OQ[w].get_v() < _OQ[w].get_t() && *zz == e ){
        ...
        _shift[_OQ[w].get_t() + _add.get_t()] = _SG.edgeEnd(e) - 1 - j;
        _add.push_back(v);
      }
      if ( *x < e ) x++;
      j++;
    ...
    while (1){   
			...
			// check *x is adjacent to all vertices in K\cap N(w) or not, 
			// one-by-one. if not, then break the loop
    ...
    while (flag){   // check *x is adjacent to all vertices in K_{\le w}. If not, then break the loop
			...
			// if *x is adjacent to all, MACE_occ[w] is not a child
      if ( i== _OQ[w].get_t() + _add.get_t() ) return (*x); 

      Z = _SG.edge_vv( _add.get_v(i-_OQ[w].get_t()) );

      while ( *_shift[i]>*x ){
        _shift[i]--;

				// reached to the end of the adjacency list of the i-th added vertex, 
				// thus no further vertex can be pass this check, 
				// and set flag to 0 not to come here again.
        if ( _shift[i] < Z ){ 
        	flag = 0; 
        	goto LOOP_END; 
        } 
...
LONG KGMACE::VBM_parent_check ( QUEUE_INT K , QUEUE_INT w){
  ...
  for (x=_SG.edgeEnd(v)-1 ; *x>w ; x--){
    ...
    if ( *x == *z ){ 
	  ...
	  // parentness
    if ( _VV.existEdge(*x,pp) ){ return (*x); }
